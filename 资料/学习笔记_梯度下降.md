...
1. **初始化参数**: $W$ 和 $B$ / $K_i$ 为0或随机值

```python
w = 0.00    #初始化斜率
b = 0.00    #初始化截距
# 或者
k = [0.0 for _ in range(X_train)]   #初始化多元线性回归的各个系数
```
2. **计算损失** : 使用方差函数计算损失(MSE)
```python
...
def compute_loss(X, y, k):
    ...
    """
    m = len(X)
    total_error = 0.00  #到目前位置和前面一样
    for i in range(m):
        prediction = sum(
            k[j] * X[i][j] 
                for j in range(len(k)))         # 多元线性回归的预测值
    #上述一串下来本质是 k[0]*X[i][0] + k[1]*X[i][1] + ... + k[n]*X[i][n]
        error = prediction - y[i]
        total_error += error ** 2
        loss = total_error / (2 * m)
    return loss
...
3. **计算梯度** : 计算损失函数对 $W$ 和 $B$ / $K_i$ 的偏导数
```python
def compute_gradients(X, y, k):
    """
    计算各个系数的偏导数
    """
    m = len(X)
    gradients = [0.0 for _ in range(len(k))]    # 初始化梯度列表
    for i in range(m):
        prediction = sum(
            k[j] * X[i][j] 
        ...
        error = prediction - y[i]
        for j in range (len(k)):
            gradients[j] += error * X[i][j]    # 计算每个系数的梯度
    # 求平均梯度
    for j in range(len(k)):
        gradients[j] /= m
    return gradients
...
4. **更新参数** : 使用以上计算的梯度更新 $W$ 和 $B$ / $K_i$
```python
def update_parameters(X, y, k, learning_rate = 0.01, epochs = 1000):
    """
    一次完整的参数更新流程
    """
    m = len(x)
    for epoch in range(epochs):
        ...
        # 2. 计算梯度
        gradients = compute_gradients(X, y, k)  #计算各个系数的梯度,详见上

        # 3. 更新参数
        for j in range(len(k)):
            k[j] = k[j] - learning_rate * gradients[j]
    return k
```